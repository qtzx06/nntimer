...
classifier_model.fc = torch.nn.Linear(classifier_model.fc.in_features, len(CLASSIFIER_CLASS_NAMES))
...
class StateManager:
    def __init__(self):
        self.latest_frame, self.current_status, self.solve_time = None, TimerState.IDLE, 0.0
    ...
    def set_status(self, status, solve_time=None):
        with self._lock:
            ...
            if solve_time is not None: self.solve_time = solve_time
    def get_status(self):
        with self._lock: return {"status": self.current_status, "time": f"{self.solve_time:.2f}"}
    ...
    for row in output:
        ...
        if class_confidence > best_confidence:
            ...
            if class_confidence > CONFIDENCE_THRESHOLD:
                detector_probs = {name: prob for name, prob in zip(DETECTOR_CLASS_NAMES, row[4:])}
                cx, cy, w, h = row[:4]
                h_orig, w_orig = frame_shape
                x1, y1 = int((cx - w / 2) * w_orig / 640), int((cy - h / 2) * h_orig / 640)
                x2, y2 = int((cx + w / 2) * w_orig / 640), int((cy + h / 2) * h_orig / 640)
...
def classify_cube_state(image_crop, model):
    ...
    classifier_probs = {name: prob.item() for name, prob in zip(CLASSIFIER_CLASS_NAMES, probabilities)}
...
def video_processing_thread():
    state, start_time, solve_time, frame_count, detection_miss_counter = TimerState.IDLE, 0, 0, 0, 0
    ...
    # --- new: custom drawing styles for mediapipe ---
    drawing_spec_dots = mp.solutions.drawing_utils.DrawingSpec(color=(0, 0, 255), thickness=3, circle_radius=3) # bright red dots
    drawing_spec_lines = mp.solutions.drawing_utils.DrawingSpec(color=(255, 255, 255), thickness=3) # bright white lines
    ...
    for i in range(3):
        ...
        if cap.isOpened():
            ...
            print(f"webcam opened successfully at index {i} with resolution {width}x{height}.")
            break
    ...
    while True:
        ret, frame = cap.read()
        if not ret: print("error: can't receive frame from webcam."); break
        ...
        if frame_count % PROCESS_EVERY_N_FRAMES == 0:
            ...
            detector_output = ort_session.run(None, {detector_input_name: detector_input})[0]
            cube_bbox, detector_probs = postprocess_detector_output(detector_output, frame.shape[:2])
            ...
            if cube_bbox:
                ...
                if has_enough_color_variation(cropped_cube, COLOR_VARIATION_THRESHOLD):
                    classifier_probs = classify_cube_state(cropped_cube, classifier_model)
                    combined_solved_confidence = (classifier_probs['solved'] * CLASSIFIER_WEIGHT) + (detector_probs['SolvedCube'] * DETECTOR_WEIGHT)
                    last_known_class_name = 'solved' if combined_solved_confidence > 0.5 else 'unsolved'
        ...
        if state == TimerState.IDLE:
            if cube_is_present and last_known_class_name == 'unsolved': state = TimerState.READY
        elif state == TimerState.READY:
            if not cube_is_present: state = TimerState.IDLE
            elif hands_present:
                wrist_pos = hand_results.multi_hand_landmarks[0].landmark[mp_hands.HandLandmark.WRIST]
                current_hand_pos = (wrist_pos.x, wrist_pos.y)
                if last_hand_pos is None: last_hand_pos, steady_start_time = current_hand_pos, current_time
                movement = math.sqrt((current_hand_pos[0] - last_hand_pos[0])**2 + (current_hand_pos[1] - last_hand_pos[1])**2)
                if movement < HAND_STEADY_THRESHOLD:
                    if current_time - steady_start_time > HAND_STEADY_DURATION_REQ:
                        state, last_landmarks = TimerState.SET, hand_results.multi_hand_landmarks
                else: steady_start_time = current_time
                last_hand_pos = current_hand_pos
            else: last_hand_pos = None
        elif state == TimerState.SET:
            if not cube_is_present: state = TimerState.IDLE
            elif not hands_present: state, start_time, solve_time = TimerState.SOLVING, current_time, 0
            elif hands_present and last_landmarks:
                total_velocity = sum(math.sqrt((hand_landmarks.landmark[lm_idx].x - last_landmarks[i].landmark[lm_idx].x)**2 + (hand_landmarks.landmark[lm_idx].y - last_landmarks[i].landmark[lm_idx].y)**2) for i, hand_landmarks in enumerate(hand_results.multi_hand_landmarks) if i < len(last_landmarks) for lm_idx in [mp_hands.HandLandmark.THUMB_TIP, mp_hands.HandLandmark.INDEX_FINGER_TIP])
            ...
            if last_known_class_name == 'solved' and hands_present and last_known_confidence > SOLVED_STATE_CONFIDENCE_THRESHOLD:
            ...
            if not hands_present: state = TimerState.READY

        state_manager.set_status(state, solve_time)

        # --- drawing logic ---
        display_text = state
        if state == TimerState.SOLVING: display_text = f"solving: {solve_time:.2f}"
        elif state == TimerState.SOLVED: display_text = f"solved: {solve_time:.2f}"

        if detection_miss_counter > DETECTION_GRACE_PERIOD: last_known_bbox = None
        if last_known_bbox:
...
@app.get("/")
async def read_root():
    with open("frontend/index.html") as f: return HTMLResponse(f.read())
@app.get("/status")
